...
export class FactureComponent implements OnInit, OnChanges {
  ...
  @Input('facture') facture: FactureReq = {} as FactureReq;
  ...
  loadFacture(facture: FactureReq): FormGroup {
    let factureForm: FormGroup = this.fb.group({
      ligneFactures: this.fb.array(
        facture.ligneFactures.map((x) => this.fb.group(x))
      ),
